...
class UserController extends Controller {
  ...
  async yzmupdate() {
    ...
    async findOpenId(){
      const { ctx,app} = this;
      const { wxOpenId } = ctx.request.body;
      const where = {}
  ...
  async addUserInfo(){
    const {ctx,app} = this;
    const {username,name,sex,age,phone,hight,weight,senFen,address,photo,wxOpenId,qq,keCheng,type,email,balance,integral,record,birthday,motto,registerTime}=ctx.request.body;
    // 用户名
    try {
      let res = await ctx.model.User.create({
      username,
  ...
  async findOneUid (){
    ...
    if(!uid) return (ctx.body ={ success: false, errCode: 30001,info: '该用户数据有误' })
    try {
      ...
      ctx.body = {success: true,errCode: '',info:'数据查找成功',data:res}
    } catch (error) {
      ctx.body = {success: false, errCode: 30001,info: '数据查找失败'}
      console.log(error);
    }
  }
  // 创建学员购买详情记录
  async coursePayJiLu(){
    const { ctx,app } = this;
    const { uid,coachId,state,courseName,userNumber,dtype,startTime,endTime } = ctx.request.body;
    try {
      const res = await ctx.model.CoursePayJiLu.create({
        uid,
        coachId,
        state,
        courseName,
      ...
      ctx.body={success:true,errCode: '',info:'数据创建成功',data:res}
    } catch (error) {
      ctx.body={success:true,errCode: 40002,info:'数据创建失败'}
  ...
  // 查询学员购买详情记录
  async findCoursePayJiLu(){
    const { ctx,app } = this;
    const {coachId,dtype,uid} = ctx.request.body;
    const where = {}
    console.log("findCoursePayJiLu",ctx.body)

    if(coachId) where.coachId = coachId
    if(dtype) where.dtype = dtype
    try {
     const shuJuONe = await ctx.model.CoursePayJiLu.findAll({
        where
      })
      ctx.body={success:true,errCode: '',info:'数据查找成功',data:shuJuONe}

      const shuJuTwo = await ctx.model.CoursePayJiLu.findAll({
        where: {
          uid:146,
          coachId:22
      ...
      ctx.body={success:true,errCode: '',info:'数据查找成功',data:shuJuTwo}
